...
export sedona_home=/home/titus/workdir/rPi/June25/sedonadev-RPi2-bacnet-mqtt-git
...
java_home=$JAVA_HOME
[ -z "$java_home" ] && java_home=/usr/lib/jvm/java-7-openjdk-i386
#[ -z "$java_home" ] && java_home=/usr/lib/jvm/jdk-8-oracle-arm32-vfp-hflt
export java_home

# check to make sure that programs we need are in the path
for p in javac gcc python
do
  if ! which $p > /dev/null
  then
