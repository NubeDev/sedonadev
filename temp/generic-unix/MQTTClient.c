...
//Titus: Initial dummy initialization
char *topic_rcd = "Aidan";
...
Cell mqtt_MqttDev_doMqttRead_Topic(SedonaVM* vm, Cell* params)
{

Cell result;
result.aval = topic_rcd;
return result;
...
int deliverMessage(Client* c, MQTTString* topicName, MQTTMessage* message)
{
    ...
    for (i = 0; i < MAX_MESSAGE_HANDLERS; ++i)
    {
        ...
        {
//Titus
//printf("############## deliverMessage1 %s ##############\n",c->messageHandlers[i].topicFilter);

            if (c->messageHandlers[i].fp != NULL)
//Titus
//printf("############## deliverMessage2 %s ##############\n",c->messageHandlers[i].topicFilter);
//Titus : Send to sedona the topic name which receives the message.
	topic_rcd = c->messageHandlers[i].topicFilter;
